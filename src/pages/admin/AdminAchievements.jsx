...
const AdminAchievements = ({ onSave }) => {
  const { profile, refreshData } = useContext(PortfolioContext);
  const [achievements, setAchievements] = useState([emptyItem('Achievement')]);
  const [participation, setParticipation] = useState([emptyItem('Participation')]);
  ...
  useEffect(() => {
    if (profile?.achievements?.length > 0) {
      const ach = profile.achievements.filter((t) => t.category === 'Achievement');
      const part = profile.achievements.filter((t) => t.category === 'Participation');
      if (ach.length) setAchievements(ach.map((t) => ({ category: 'Achievement', period: t.period || '', title: t.title || '', subtitle: t.subtitle || '' })));
      if (part.length) setParticipation(part.map((t) => ({ category: 'Participation', period: t.period || '', title: t.title || '', subtitle: t.subtitle || '' })));
    }
  }, [profile]);
  ...
  const handleSave = async (e) => {
    ...
    try {
      const res = await authFetch('/api/profile', {
        ...
        body: JSON.stringify({ achievements: [...achievements, ...participation] }),
      });
      if (res.ok) { showToast('Achievements saved!'); refreshData(); onSave?.(); }
      ...
      <div className="space-y-5">
        {items.map((item, idx) => (
          <div key={idx} className="border border-gray-100 rounded-xl p-4 bg-gray-50 relative">
            ...
            <input className={`${inputStyle} mb-3`} placeholder="2023" value={item.period} onChange={(e) => handleChange(setter, idx, 'period', e.target.value)} />
    ...
    <>
      ...
      <form onSubmit={handleSave} className="max-w-4xl space-y-6">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {renderColumn(achievements, setAchievements, 'Achievement', '🏆', 'Achievements')}
          {renderColumn(participation, setParticipation, 'Participation', '🚀', 'Participation')}
        </div>
        <button type="submit" disabled={saving} className="w-full bg-olivia-green text-white font-bold py-4 rounded-full hover:bg-olivia-gold hover:text-black transition-colors shadow-md disabled:opacity-60">{saving ? 'Saving...' : 'Save Achievements'}</button>
      </form>
    </>
  );
};
